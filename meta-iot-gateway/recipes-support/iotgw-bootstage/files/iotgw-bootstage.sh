...
if [ -z "${bootstage_root}" ]; then
    log_msg "bootstage DT node not found; verify CONFIG_BOOTSTAGE_FDT=y and iotgw_bootstage=1"
    exit 0
fi
...
while IFS= read -r idx; do
    ...
    log_msg "stage=${name} kind=${kind} us=${value}"
...
if [ "$record_count" -eq 0 ]; then
    log_msg "bootstage node exists but contains no usable records"
    exit 0
fi
...
log_msg "summary records=${record_count} max_mark_us=${max_mark} sum_mark_us=${sum_mark}"
